...
def resample_to_1mm(img: nib.Nifti1Image, mask: nib.Nifti1Image) -> tuple[nib.Nifti1Image, nib.Nifti1Image]:
    img_r  = ants_to_nib(ants.resample_image(nib_to_ants(img),  (1, 1, 1), use_voxels=False, interp_type=4))
    mask_r = ants_to_nib(ants.resample_image(nib_to_ants(mask), (1, 1, 1), use_voxels=False, interp_type=1))
    return img_r, mask_r
...
def process_synthseg_batch(tasks: list[tuple]) -> list[dict]:
    ...
    for task in tasks:
        ...
        if (seg_path.exists() and dseg_tsv.exists()
            ...
            pending.append((name, input_path, seg_path, dseg_tsv, volumes_tsv, qc_tsv,
                            synthseg_threads, cpu_only))
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_synthseg(
                input_paths, seg_paths, vol_csvs, qc_csvs,
                synthseg_threads, cpu_only,
...
def _run_synthseg_stage(
    ...
    synthseg_file_tasks = [
        (f, base_dir, synthseg_dir, synthseg_threads, cpu_only)
        for f in files
    ]
    ...
    log.info(
        ...
        len(synthseg_file_tasks), synthseg_workers, len(synthseg_batches), n_gpus, cpu_only,
    )
...
def main() -> None:
    ...
    if args.synthseg:
        ...
        _run_synthseg_stage(
            synthseg_input_files, out_dir, synthseg_dir, synthseg_workers, n_gpus,
            args.synthseg_threads, args.cpu, log_file, log_dir,
        )
